...
export default function DashboardPage() {
  const { assets, sectors, profile, authLoading, isAdmin } = useInventory();
  ...
  // Agrupamento de cálculos pesados usando useMemo para performance
  const stats = useMemo(() => {
    ...
    const sortedAssets = [...assets].sort((a, b) => a.sector.localeCompare(b.sector) || a.name.localeCompare(b.name));

    return {
      totalAssets, globalConfirmed, globalMoved, globalPending,
      condBom, condRuim, condInservivel, condNaoAvaliado,
      globalDone, globalProgress, sectorStats, topCategories,
      sortedAssets
    };
  }, [assets, sectors]);
  ...
  return (
    <div className="dashboard-view fade-in">
      <div className="stats-grid">
        <div className="stat-card"><div className="stat-value">{stats.totalAssets}</div><div className="stat-label">Total GERAL</div></div>
        <div className="stat-card"><div className="stat-value" style={{ color: '#34D399' }}>{stats.globalDone}</div><div className="stat-label">Verificados</div></div>
        <div className="stat-card"><div className="stat-value" style={{ color: '#FBBF24' }}>{stats.globalMoved}</div><div className="stat-label">Movimentados</div></div>
        <div className="stat-card"><div className="stat-value" style={{ color: '#94A3B8' }}>{stats.globalPending}</div><div className="stat-label">Pendentes</div></div>
      </div>

      <div className="glass-panel" style={{ padding: '20px', marginBottom: '20px' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: '600', marginBottom: '16px' }}>Progresso Total da Auditoria</h3>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
          <span style={{ fontSize: '1.2rem', fontWeight: '700', color: 'var(--secondary)' }}>{stats.globalProgress}%</span>
          <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{stats.globalDone} de {stats.totalAssets} itens</span>
        </div>
        <div className="progress-bar"><div className="progress-fill" style={{ width: `${stats.globalProgress}%` }} /></div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '20px', marginBottom: '20px' }}>
        ...
        <div className="chart-bar-horizontal">
          {stats.condBom > 0 && <div className="chart-segment" style={{ width: `${(stats.condBom / stats.globalDone) * 100}%`, background: '#34D399' }} />}
          {stats.condRuim > 0 && <div className="chart-segment" style={{ width: `${(stats.condRuim / stats.globalDone) * 100}%`, background: '#FBBF24' }} />}
          {stats.condInservivel > 0 && <div className="chart-segment" style={{ width: `${(stats.condInservivel / stats.globalDone) * 100}%`, background: '#EF4444' }} />}
          {stats.condNaoAvaliado > 0 && stats.globalDone > 0 && <div className="chart-segment" style={{ width: `${(stats.condNaoAvaliado / stats.globalDone) * 100}%`, background: '#334155' }} />}
        </div>
        <div className="chart-legend" style={{ marginTop: '12px', display: 'flex', flexWrap: 'wrap', gap: '12px', fontSize: '0.8rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '10px', height: '10px', background: '#34D399', borderRadius: '2px' }} /> Bom ({stats.condBom})</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '10px', height: '10px', background: '#FBBF24', borderRadius: '2px' }} /> Ruim ({stats.condRuim})</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '10px', height: '10px', background: '#EF4444', borderRadius: '2px' }} /> Inservível ({stats.condInservivel})</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><div style={{ width: '10px', height: '10px', background: '#334155', borderRadius: '2px' }} /> S/ Avaliação ({stats.condNaoAvaliado})</div>
        </div>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '20px', marginBottom: '40px' }}>
        ...
        <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
          Visão gerencial completa de todos os {stats.totalAssets} patrimônios cadastrados na unidade.
        </p>
            ...
            >
              ...
              <div className="report-item-detail" style={{ display: 'flex', justifyContent: 'space-between', marginTop: '6px' }}>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  {item.condition && <span>🏷️ {item.condition}</span>}
